...
HOOKS:=pre-commit
LDFLAGSW:=-ldflags "-X github.com/feixao/go-shopping/warehouse/internal/platform/config.Version=${VERSION}"
LDFLAGSC:=-ldflags "-X github.com/feixao/go-shopping/catalog/internal/platform/config.Version=${VERSION}"
LDFLAGSS:=-ldflags "-X github.com/feixao/go-shopping/shipping/internal/platform/config.Version=${VERSION}"
LDFLAGSA:=-ldflags "-X github.com/feixao/go-shopping/api/internal/platform/config.Version=${VERSION}"
...
api: clean api-lint
	...
	@cd api/cmd/apid && go get && CGO_ENABLED=0 go build ${LDFLAGSW} -a -installsuffix cgo -o apid main.go
...
warehouse: clean warehouse-lint
	...
	@cd warehouse/cmd/warehoused && go get && CGO_ENABLED=0 go build ${LDFLAGSW} -a -installsuffix cgo -o warehoused main.go
...
catalog: clean catalog-lint
	...
	@cd catalog/cmd/catalogd && go get && CGO_ENABLED=0 go build ${LDFLAGSC} -a -installsuffix cgo -o catalogd main.go
...
shipping: clean shipping-lint
	...
	@cd shipping/cmd/shippingd && go get && CGO_ENABLED=0 go build ${LDFLAGSS} -a -installsuffix cgo -o shippingd main.go
